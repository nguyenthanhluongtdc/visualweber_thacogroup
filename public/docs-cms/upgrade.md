...
+ `Language::registerModule({PLUGIN}_MODULE_SCREEN_NAME);` to `Language::registerModule(\Platform\{Plugin}\Models\{Plugin}::class)`
+ `SeoHelper::registerModule({PLUGIN}_MODULE_SCREEN_NAME);` to `SeoHelper::registerModule(\Platform\{Plugin}\Models\{Plugin}::class)`
+ Change
```php
config([
...
```php
SlugHelper::registerModule(\Platform\{Plugin}\Models\{Plugin}::class);
```
...
- Run `composer install` to update vendor packages.
- Run `php artisan migrate` to update database.
- Deprecated classes: Platform\Base\Tables\TableAbstract & Platform\Base\Tables\TableBuilder will be removed in the next version, 
please change to use Platform\Table\Abstracts\TableAbstract & Platform\Table\TableBuilder
