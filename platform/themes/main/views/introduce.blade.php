<section class="banner-introduce">
    <img class=" h-auto w-100"
        src="{{ get_field($page, 'about_us_banner') ? get_image_url(get_field($page, 'about_us_banner')) : Theme::asset()->url('images/introduce/banner-introduce.jpg') }}"
        alt="banner">
</section>
...
<section class="about-us mt-40">
    ...
    <div class="container-customize">
        ...
        <div class="leader-of-us-wrapper mt-40">
            ...
            @if (has_field($page, 'council'))
                @foreach (has_field($page, 'council') as $key => $item_council)
                    ...
                    <div class="admin-content {{ $key == 0 ? 'top' : 'bottom' }}" data-aos="fade-up"
                        data-aos-duration="700" data-aos-delay="50" class="aos-init aos-animate">
                        @foreach (has_sub_field($item_council, 'council_member') as $item_member)
                            <div class="item-member">
                                <div class="admin-item"
                                    data-target="#{{ Str::slug(has_sub_field($item_member, 'name')) }}"
                                    data-toggle="modal">
                                    <img src="{{ get_image_url(has_sub_field($item_member, 'image')) }}"
        ...
        <div class="achievement-wrapper mt-40 mb-60">
            <div class="achievement-tab-title">
                <div class="achievement__title mt-40 " data-aos="fade-right" data-aos-duration="700" data-aos-delay="50"
                    class="aos-init aos-animate">
                    <img src="{{ Theme::asset()->url('images/introduce/arrow.png') }}" alt="">
                        ...
                        @if (has_field($page, 'achivement'))
                            @foreach (has_field($page, 'achivement') as $key => $item_tab)
                                <li class="__tabs__item " role="achievement">
                                    <a class="__tabs__link nav-link {{ $key == 0 ? 'active' : '' }}"
                                        id="achievement-company-{{ $key }}" data-toggle="tab" role="tab"
                                        aria-controls="achievement-{{ $key }}" aria-selected="true"
                                        href="#achievement-{{ $key }}"
                                        title="{{ has_field($page, 'title_tab') }}">
                ...
                @if (has_field($page, 'achivement'))
                    @foreach (has_field($page, 'achivement') as $key => $item_tab_content)
                        <div class="tab-pane fade {{ $key == 0 ? 'active show' : '' }}"
                            id="achievement-{{ $key }}" role="tabpanel" aria-labelledby="field-1-tab">
                            <div class="content-tab" data-aos="fade-right" data-aos-duration="700"
                                data-aos-delay="50" class="aos-init aos-animate">
    ...
    <div class="develop-wrapper">
        <div class="develop__title pb-40 container-customize" data-aos="fade-right" data-aos-duration="700"
            data-aos-delay="50" class="aos-init aos-animate">
            <img class="img-blue" src="{{ Theme::asset()->url('images/introduce/arrow.png') }}" alt="icon">
            <img class="img-white" src="{{ Theme::asset()->url('images/introduce/icon-arrow-white.png') }}"
                alt="icon" style="display:none">
            <h2 class="font50 big-title"> {!! has_field($page, 'title_develop') !!}</h2>
        </div>
        <div class="develop-banner lazyloaded "
            style="background-image:url('{{ get_field($page, 'bg_develop') ? get_image_url(get_field($page, 'bg_develop')) : Theme::asset()->url('images/introduce/1.jpg') }}')">
                ...
                <div class="develop-content__slider">
                                        ...
                                        @foreach (has_field($page, 'content_slide') as $item_develop_content)
                                            <div class="thumbnail-image mCustomScrollbar" data-mcs-theme="dark">
                                                <div class="image-slider">
                                                    ...
                                                    <img src="{{ get_sub_field($item_develop_content, 'image') ? get_image_url(get_sub_field($item_develop_content, 'image')) : '' }}"
                    ...
                    <div class="carousel-controls transparent-controls">
                        <button class="play active"><img
                                src="{{ Theme::asset()->url('images/introduce/play-button-1.png') }}"
                                alt="icon"></button>
                        <button class="pause"><img
                                src="{{ Theme::asset()->url('images/introduce/pause-1.png') }}" alt="icon"></button>
                    </div>
                </div>
            </div>
        </div>
...
@if (has_field($page, 'council'))
    @foreach (has_field($page, 'council') as $key => $item_council_2)
        @foreach (has_sub_field($item_council_2, 'council_member') as $item_member)
            <div class="modal fade modal_admin" id="{{ Str::slug(has_sub_field($item_member, 'name')) }}"
                tabindex="-1" role="dialog" aria-labelledby="info_admin_modallLabel" aria-hidden="true">
