...
<div {!! clean($options) !!} class="list-media-menu" data-aos="fade-up" data-aos-duration="100" data-aos-delay="50" class="aos-init aos-animate">
    ...
    <a href="{{ $key!=5 ? 'javascript:;' : $row->url }}" target="{{ $row->target }}" class=" {{$key!=5? 'item_link_media' :''}} list-group-item  font18 font-myria-bold {{ $row->active ? "active" : ""}}" data-category={{$row->reference_id}}>
