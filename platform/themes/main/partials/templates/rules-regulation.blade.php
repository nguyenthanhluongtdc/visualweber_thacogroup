     ...
     <div class="info-item" data-aos="fade-left" data-aos-duration="500" data-aos-delay="50"
             ...
             <div class="date">
                 <p>
                     ...
                     <sup class="">-{{ $item->created_at->format('m') }}</sup>
                                        ...
                                        <p class="
                         date-year fon16 text-center">{{ $item->created_at->format('Y') }}
                 </p>
             </div>
         </div>
         <div class="info-right">
             <h3>
                 ...
                 <a href="{{ count(has_field($item, 'repeater_file_post_investor')) == 1 ? get_object_image(has_sub_field(has_field($item, 'repeater_file_post_investor')[0], 'file')) : '' }}"
                     class="font25 text-justify {{ count(has_field($item, 'repeater_file_post_investor')) > 1 ? 'itemdown-show' : '' }} "
                     target="_blank">
                     {!! $item->name !!}
                 </a>
